...
bool intcyl(const glm::vec3 &raybase, const glm::vec3 &raycos, float radius, glm::vec3 *in, glm::vec3 *out)
{
	...
	glm::vec3		RC;		/* Ray base to cylinder base	*/
	float		d;		/* Shortest distance between	*/
	glm::vec3		axis(0.f, 0.f, 1.f);/*   the ray and the cylinder	*/
	...
	RC.y = raybase.y;
	RC.z = raybase.z;
	n = glm::cross(raycos, axis);

	ln = length_int(n);

	if (ln == 0.f) {	/* ray parallel to cyl	*/
		d = glm::dot(RC, axis);
		D.x = RC.x;
		D.y = RC.y;
		D.z = RC.z - d;
	...
	n = glm::normalize(n);
	d = fabs(glm::dot(RC, n));		/* shortest distance	*/
	hit = (d <= radius);
	...
	if (hit) {				/* if ray hits cylinder */
		...
		O = glm::cross(n, axis);
		O = glm::normalize(O);
		s = fabs(sqrt(radius*radius - d*d) / glm::dot(raycos, O));
		*in = RC + (t - s) * raycos;			/* exiting  distance	*/
		*out = RC + (t + s) * raycos;			/* exiting  distance	*/
	}
...
{
	float r_div_h_deg2 = r_div_h*r_div_h;
	float a = dir.x*dir.x + dir.y*dir.y - dir.z*dir.z * r_div_h_deg2;
	float b = dir.x*ori.x + dir.y*ori.y - dir.z*ori.z * r_div_h_deg2;
	float c = ori.x*ori.x + ori.y*ori.y - ori.z*ori.z * r_div_h_deg2;

	float d_div_4 = b*b - a*c;

	if (d_div_4 < 0)
		return false;

	float x1 = (-b + sqrt(d_div_4)) / a;
	float x2 = (-b - sqrt(d_div_4)) / a;
	*in = ori + dir * x1;
	*out = ori + dir * x2;
	return true;
}
